...
export default function (accessToken) {
    ...
    check(res, {
        'product stocks negative status is 200': (r) => r.status === 200,
    });
...
export function handleSummary(data) {
    ...
    const reportName = `reports/ProductStocks_Negative_${timestamp}.html`;

    return {
        [reportName]: htmlReport(data, { title: `Product Stocks (Negative) - ${BASE_URL}` }),
    };
}
